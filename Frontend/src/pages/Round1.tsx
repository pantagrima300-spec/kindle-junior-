...
const Round1 = () => {
  ...
  const langFromState = location.state?.language === "python" ? "Python" : (location.state?.language === "c" ? "C" : "");
      ...
      try {
        ...
        if (attemptRes.status === 400) {
            ...
            if (errText.includes("invalid language") || errText.includes("language_required")) {
                navigate("/round-1-guidelines", { replace: true });
                return;
            }
            throw new Error(
        ...
        <div className="question-meta">
          ...
          <div className="question-language">
            {currentQuestion.domain?.toLowerCase() === "python" ? (
              <>
                <span className="language-icon python-icon">ðŸ</span>
                <span>Python</span>
              </>
...
.c-icon {
  background: rgba(36, 156, 255, 0.15);
  border: 1px solid rgba(36, 156, 255, 0.4);
  color: #249cff;
}
*/
